...
class OpenFile {
  public:
    ...
    OpenFile(int f) { file = f; currentOffset = 0; type = 0; }			// open default the file 

    OpenFile(int f, int m_type) {file = f; currentOffset = 0; type = m_type; } 	// open file with type
...
class OpenFile {
  public:
    // open file with type
    int type; 				// 0- read & write; 1- only read, 2- stdin, 3- stdout.


    OpenFile(int sector);		// Open a file whose header is located
					// at "sector" on the disk

    OpenFile(int sector, int m_type);   // Open a file with type
